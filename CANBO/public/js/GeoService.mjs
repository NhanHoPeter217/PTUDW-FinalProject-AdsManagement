...
async function getDataFromLatLng(map, latlng) {
    ...
    if (place_id == '' || address == '' || district == '') {
        if (place_id == '')
            console.log('[Google Maps API] Fail to get place_id from latlng', latlng);
        if (address == '') console.log('[Google Maps API] Fail to get address from latlng', latlng);
        if (district == '')
            console.log('[Google Maps API] Fail to get district from latlng', latlng);
        return null;
    }
